...
describe("the clientState() function", () => {
  it("if the last state emission was not 'start', it should throw INVALID_CALL and emit nothing", () => {
    ...
    // Check the error
    expect(() => {
      wrapper.clientState("some client");
    }).toThrow(
      new Error(
  ...
  it("if the call was valid and the transport returns an invalid type, it should throw TRANSPORT_ERROR and emit transportError", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Check the return value
    transport.clientState = () => 123;
    expect(() => {
      wrapper.clientState("some client");
    }).toThrow(new Error("TRANSPORT_ERROR: Transport returned invalid state."));
  ...
  it("if the call was valid and the transport returns an invalid string, it should throw TRANSPORT_ERROR and emit transportError", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Check the return value
    transport.clientState = () => "bad string";
    expect(() => {
      wrapper.clientState("some client");
    }).toThrow(new Error("TRANSPORT_ERROR: Transport returned invalid state."));
  ...
  it("if the call was valid, the client was understood to be connected, and transport returns disconnected, it should throw TRANSPORT_ERROR and emit transportError", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Get the client into connected state
    ...
    // Check the return value
    transport.clientState = () => "disconnected";
    expect(() => {
      wrapper.clientState("some client");
    }).toThrow(
      new Error("TRANSPORT_ERROR: Transport returned unexpected state.")
    );
  ...
  it("if the call was valid, the client was understood to be disconnected, and transport returns connected, it should throw TRANSPORT_ERROR and emit transportError", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Check the return value
    transport.clientState = () => "connected";
    expect(() => {
      wrapper.clientState("some client");
    }).toThrow(
      new Error("TRANSPORT_ERROR: Transport returned unexpected state.")
  ...
  it("if both sides are valid and client was connected, it should return 'connected' and emit nothing", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Get the client into the connected state
    transport.emit("connect", "some client");
    ...
    // Check the return value
    transport.clientState = () => "connected";
    expect(wrapper.clientState("some client")).toBe("connected");

    // Check the listener
    expect(listener.mock.calls.length).toBe(0);
  });

  it("if both sides are valid and client was disconnected, it should return 'disconnected' and emit nothing", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Check the return value
    transport.clientState = () => "disconnected";
    expect(wrapper.clientState("some client")).toBe("disconnected");
...
describe("the clients() function", () => {
  ...
  it("if the call was valid and the transport returns an invalid type, it should throw TRANSPORT_ERROR and emit transportError", () => {
    ...
    // Check the return value
    transport.clients = () => 123;
    expect(() => {
      wrapper.clients();
    }).toThrow(
      new Error("TRANSPORT_ERROR: Transport returned invalid client list.")
    );
  ...
  it("if the call was valid and the transport returns an invalid element type, it should throw TRANSPORT_ERROR and emit transportError", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
  ...
  it("if the call was valid and the transport returns a non-matching list of clients, it should throw TRANSPORT_ERROR and emit transportError", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Check the return value
    transport.clients = () => ["client1", "client3"];
    expect(() => {
      wrapper.clients();
    }).toThrow(
      new Error("TRANSPORT_ERROR: Transport returned unexpected client list.")
    );
  ...
  it("if the call was valid, it should return the results and emit nothing", () => {
    // Set up the transport
    const transport = emitter({
      on: () => {},
      state: () => "stopped",
    ...
    // Check the return value
    transport.clients = () => ["client2", "client1"]; // Different order from _clientIds
    expect(wrapper.clients()).toEqual(["client2", "client1"]);
