      ...
      describe("if there is no disconnect/stoppage during processing", () => {
        describe("if next the app calls fores.success() and then fores.success()", () => {
          ...
          it("should have appropriate return values/errors", () => {
            ...
            expect(() => {
              fores.success({ feed: "data" })
              ;}).toThrow("ALREADY_RESPONDED: The feedOpenResponse.success() or feedOpenResponse.failure() method has already been called.");
          });
        });
        ...
        describe("if next the app calls fores.failure() and then fores.success()", () => {
          ...
          it("should have appropriate return values/errors", () => {
            ...
            expect(fores.failure("SOME_ERROR", { error: "data"})).toBeUndefined();
  ...
  describe("if there is no feedClose listener", () => {
    describe("if the feed is open", () => {
      ...
      it("should emit no events", () => {
        ...
        harn.makeServerStarted();
        const cid = harn.makeClient("some_tcid");
        harn.makeFeedOpen("some_tcid", "some_feed", {feed: "args"}, {feed: "data"});

        harn.server.emit = jest.fn(); // Can't create a listener
        harn.transport.emit("message", "some_tcid", JSON.stringify({
          MessageType: "FeedClose",
          FeedName: "some_feed",
          FeedArgs: { feed: "args" }
        }))
    ...
    describe("if the feed is open", () => {
      describe("if next the app calls fcres.success() and then again", () => {
        ...
        it("should have appropriate errors and return values", () => {
          ...
          expect(()=>{
            fcres.success();
          }).toThrow(new Error("ALREADY_RESPONDED: The feedCloseResponse.success() method has already been called."))
        });
      });
